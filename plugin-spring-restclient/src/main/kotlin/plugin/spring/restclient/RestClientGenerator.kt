...
private fun StringBuilder.appendBody(endpoint: RestClientEndpointModel) {
    ...
    when {
        ...
        body.optional ->
            append(
                "                body = ${body.parameterName}?.let { value -> dev.akif.tapik.plugin.spring.restclient.RestClientRequestBody(${body.definitionAccess}.mediaType, ${body.definitionAccess}.format.encode(value)) }"
            )
        else ->
            append(
                "                body = dev.akif.tapik.plugin.spring.restclient.RestClientRequestBody(${body.definitionAccess}.mediaType, ${body.definitionAccess}.format.encode(${body.parameterName}))"
            )
    }
}
...
private fun RestClientOutput.bodyMediaTypeSelection(endpoint: RestClientEndpointModel): String {
    ...
    return "dev.akif.tapik.plugin.spring.restclient.selectResponseBodyMediaType(response = response, offered = $offered, allowsNoBody = $allowsNoBody, endpointId = ${endpoint.id.kotlinString()})"
}
...
private fun StringBuilder.appendFixedHeaderHelper() {
    ...
    appendLine("        response: dev.akif.tapik.plugin.spring.restclient.RestClientResponse,")
