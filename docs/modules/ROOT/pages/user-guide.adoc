...
| Endpoint DSL
| +implementation("dev.akif.tapik:core:0.2.2")+

| Codecs
| +implementation("dev.akif.tapik:codec:0.2.2")+

| Jackson integration
| +implementation("dev.akif.tapik:jackson:0.2.2")+

| Gradle plugin
| +id("dev.akif.tapik.plugin.gradle") version "0.2.2"+
|===

[NOTE]
====
The version numbers match the repository snapshot (+version=0.2.2+). Replace them with the release you are consuming.
====
...
dependencies {
    implementation("dev.akif.tapik:core:0.2.2")
    implementation("dev.akif.tapik:jackson:0.2.2") // for JSON helpers
}
