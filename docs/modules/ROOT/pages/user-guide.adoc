...
| Endpoint DSL
| +implementation("dev.akif.tapik:core:0.2.1")+

| Codecs
| +implementation("dev.akif.tapik:codec:0.2.1")+

| Jackson integration
| +implementation("dev.akif.tapik:jackson:0.2.1")+

| Gradle plugin
| +id("dev.akif.tapik.plugin.gradle") version "0.2.1"+
|===

[NOTE]
====
The version numbers match the repository snapshot (+version=0.2.1+). Replace them with the release you are consuming.
====
...
dependencies {
    implementation("dev.akif.tapik:core:0.2.1")
    implementation("dev.akif.tapik:jackson:0.2.1") // for JSON helpers
}
