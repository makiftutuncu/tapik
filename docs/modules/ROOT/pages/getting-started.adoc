...
| +implementation("dev.akif.tapik:spring-restclient:0.2.6")+

| Generate Spring WebMVC controllers
| +implementation("dev.akif.tapik:spring-webmvc:0.2.6")+
|===
...
* +basePackage+ should point to the root package containing your +API+ implementations with +val+ properties returning +HttpEndpoint+.
...
TIP: Keep endpoint properties on concrete +API+ objects/classes; helpers can live in separate interfaces, but only +API+ implementors are scanned.
...
Implementing the +API+ interface is the convention to access the endpoint DSL and keep endpoints grouped (e.g., +ProductEndpoints.getProduct+).
