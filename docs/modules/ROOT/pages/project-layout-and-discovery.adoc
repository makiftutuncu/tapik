...
The generation flow starts from compiled bytecode on the analysis classpath.
...
In the `Gradle` plugin that means tapik scans:

* the current module's main compiled classes directory,
* compiled class directories from sibling subprojects in the same build,
* dependency JARs on the runtime classpath.
...
tapik then scans compiled classes and dependency JAR entries under that package and looks for concrete types assignable to `API`.
...
The shared generation engine is already separated from Gradle-specific wiring, and the Spring integrations now share their common HTTP mapping helpers through `common-spring`. That keeps the build integration boundary clear while also avoiding repeated Spring support code in each generator module.
