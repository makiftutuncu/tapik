...
| Single body without headers
| +Response0<T>+

| Single body with headers
| +ResponseN<T, Header1, ...>+ (where +N+ matches the header arity)

| No body, headers only
| +ResponseWithoutBodyN<Header1, ...>+

| Multiple outputs
| +OneOfK<...>+ wrapping the corresponding +ResponseX+ type
|===
