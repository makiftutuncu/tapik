...
| Status: 404 NotFound | - | `ProblemDetails` named "problem" as `application/json` |
----
...
. Depend on +dev.akif.tapik:common-plugin+ to reuse metadata types.
. Implement +TapikGenerator+ and register a new +id+.
. Traverse +HttpEndpointMetadata+ to produce your desired output. You can start from the source at +common-plugin/src/main/kotlin/plugin/MarkdownDocumentationGenerator.kt+.
