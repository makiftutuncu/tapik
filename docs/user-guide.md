...
| --- | --- |
| Endpoint DSL | `implementation("dev.akif.tapik:core:0.1.0")` |
| Codecs | `implementation("dev.akif.tapik:codec:0.1.0")` |
| Jackson integration | `implementation("dev.akif.tapik:jackson:0.1.0")` |
| Gradle plugin | `id("dev.akif.tapik.plugin.gradle") version "0.1.0"` |

!!! note
    The version numbers match the repository snapshot (`version=0.1.0`). Replace them with the release you are consuming.
...
dependencies {
    ...
    implementation("dev.akif.tapik:jackson:0.1.0") // for JSON helpers
}
