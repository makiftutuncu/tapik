...
private fun StringBuilder.appendBody(endpoint: RestClientEndpointModel) {
    ...
    when {
        ...
        body.optional ->
            append(
                "                body = ${body.parameterName}?.let { value -> dev.akif.tapik.spring.restclient.RestClientRequestBody(${body.definitionAccess}.mediaType, ${body.definitionAccess}.format.encode(value)) }"
            )
        else ->
            append(
                "                body = dev.akif.tapik.spring.restclient.RestClientRequestBody(${body.definitionAccess}.mediaType, ${body.definitionAccess}.format.encode(${body.parameterName}))"
            )
    }
}
